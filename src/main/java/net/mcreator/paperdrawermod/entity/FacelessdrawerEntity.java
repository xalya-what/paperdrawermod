...
import net.mcreator.paperdrawermod.procedures.PaperfacelessspawnProcedure;
import net.mcreator.paperdrawermod.init.PaperdrawermodModEntities;

import java.util.Set;

@Mod.EventBusSubscriber
public class FacelessdrawerEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(PaperdrawermodModEntities.FACELESSDRAWER.get(), 20, 4, 4));
	}

	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.WHITE,
			ServerBossEvent.BossBarOverlay.PROGRESS);

	public FacelessdrawerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PaperdrawermodModEntities.FACELESSDRAWER.get(), world);
	}

	public FacelessdrawerEntity(EntityType<FacelessdrawerEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		PaperfacelessspawnProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
	}
	...
	public static void init() {
		SpawnPlacements.register(PaperdrawermodModEntities.FACELESSDRAWER.get(), SpawnPlacements.Type.ON_GROUND,
