...
public class PaperBiomeFruitDecorator extends CocoaDecorator {
	public static final PaperBiomeFruitDecorator INSTANCE = new PaperBiomeFruitDecorator();
	public static com.mojang.serialization.Codec<PaperBiomeFruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("paper_biome_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public PaperBiomeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, PaperdrawermodModBlocks.PAPER_BLOCK.get().defaultBlockState());
						}
					}
				}
			});
