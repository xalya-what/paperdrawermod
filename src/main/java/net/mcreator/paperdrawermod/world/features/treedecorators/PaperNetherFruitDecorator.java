...
public class PaperNetherFruitDecorator extends CocoaDecorator {
	public static final PaperNetherFruitDecorator INSTANCE = new PaperNetherFruitDecorator();
	public static com.mojang.serialization.Codec<PaperNetherFruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("paper_nether_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public PaperNetherFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, PaperdrawermodModBlocks.PAPERBLOCK_NETHER.get().defaultBlockState());
						}
					}
				}
			});
